...
namespace ogre {
    ...
    const int           ModuleVideo::FBO_WIDTH = 1080;//1541;//1600;//2400;//1280;//;
    const int           ModuleVideo::FBO_HEIGHT = 810;//720;//560;//600;//720;
...
ModuleBase<gl::Texture2dRef>(name, origin, size, nInput, nOutput){
    if(!ModuleVideo::UP){
        ModuleVideo::UP = gl::Texture::create(loadImage(loadAsset( getAssetPath("textures/arrowUp.png"))));
    }
    if(!ModuleVideo::DOWN){
        ModuleVideo::DOWN = gl::Texture::create(loadImage(loadAsset( getAssetPath("textures/arrowDown.png"))));
    }
