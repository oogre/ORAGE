...
Button::Button( std::string name, bool value, const Format &format, const ci::gl::TextureRef &textureRef )
	: ControlWithLabel(), mUseRef( false ), mValueRef( new bool( value ) ), mValue( value ), mFormat( format ), mCallbackFn( nullptr ), mTextureRef(textureRef)
{
	setName( name );
...
Button::Button( std::string name, bool *value, const Format &format, const ci::gl::TextureRef &textureRef )
	: ControlWithLabel(), mUseRef( true ), mValueRef( value ), mValue( value ), mFormat( format ), mCallbackFn( nullptr ), mTextureRef(textureRef)
{
	setName( name );
	setSize( vec2( mFormat.mSize, mFormat.mSize ) );
	setValue( *value );
