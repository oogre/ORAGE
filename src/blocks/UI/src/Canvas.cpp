...
Canvas::Canvas( std::string name, const app::WindowRef &window )
	: Control(), mWindowRef( window ), mCanvasWidth( CanvasWidth ), mSliderHeight( SliderHeight ), mGraphHeight( GraphHeight ), mButtonDimension( ButtonDimension ), mSpacerHeight( SpacerHeight ), mDirection( Direction::SOUTH ), mAlignment( Alignment::LEFT ), mFontsize( FontSize::MEDIUM )
{
	setName( name );
...
void Canvas::removeSubView(std::string subViewName){
    View::removeSubView(subViewName);
    auto found = mLastAddedSubViews.begin();
    while(found != mLastAddedSubViews.end()){
        if((*found)->getName() == subViewName){
            break;
        }
        found++;
    }
    if(found != mLastAddedSubViews.end()){
        mLastAddedSubViews.erase(found);
    }
};
