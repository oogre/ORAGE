...
#ifndef MidiFighter_hpp
#define MidiFighter_hpp
...
*/
namespace ogre {
    class MidiFighter : public ModuleCommon{
        
        typedef const std::function<void(int,int,int)> CallbackType;
        vector<CallbackType> callbacks;
        gl::Context * mMainWinCtx;
        
        MidiFighter(string name, JsonTree data, vec2 origin, vec2 size, gl::Context * mMainWinCtx);
    ...
    public:
        ...
        typedef std::shared_ptr<class MidiFighter> MidiFighterRef;
        
        static MidiFighterRef create( const std::string name, vec2 origin, gl::Context * mMainWinCtx, JsonTree data = JsonTree())
        {
            MidiFighter::COUNT++;
            return MidiFighterRef( new MidiFighter( name, data, origin, vec2(WIDTH*0.8, 280), mMainWinCtx ) );
        }
        
        virtual JsonTree getData() override {
            {
                JsonTree obj = ModuleCommon::getData();
                obj.addChild(JsonTree("type", "MidiFighter"));
